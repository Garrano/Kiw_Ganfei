...
for i, b in enumerate(BLOCOS):
    ...
    # ONDE esta a quebra, e nao so quanto vale o degrau. O degrau e
    # media(2025-26) menos media(2017-2024): um colapso em 2024 entra na
    # PROPRIA linha de base, dilui-se, e mesmo assim pontua como degrau
    # grande. Sem esta coluna, cinco blocos que cairam em 2024 encabecam a
    # tabela e leem-se como exemplos piores do acontecimento de 2025-26 --
    # que nao sao. A quebra e a maior queda de cena para cena no desvio.
    _dif = np.diff(dev)
    _j = int(np.argmin(_dif))
    b["quebra_ano"] = DATAS[_j + 1][:4]
    b["quebra_queda"] = float(_dif[_j])
    b["quebra_no_acontecimento"] = bool(b["quebra_ano"] >= "2025")
...
print()
print("%-12s %8s %7s %10s %11s %7s %9s %s"
      % ("CUL_ID", "ENT", "ha", "degrau", "percentil", "quebra", "queda", ""))
for b in sorted(BLOCOS, key=lambda z: z["degrau"]):
    pct = 100.0 * np.mean(degraus <= b["degrau"])
    marca = "  <-- a exploração" if b["ent"] == ENT_POMAR else ""
    if not b["quebra_no_acontecimento"]:
        marca += "  [quebra fora de 2025-26]"
    print("%-12s %8s %7.2f %+10.4f %10.0f %% %7s %+9.3f%s"
          % (b["cul"], b["ent"], b["ha"], b["degrau"], pct,
             b["quebra_ano"], b["quebra_queda"], marca))

# --- censo do ano da quebra --------------------------------------------------
# O que esta tabela impede: citar os blocos com degrau mais negativo como
# "vizinhos muito piores" no acontecimento de 2025-26 sem verificar o ano.
print()
print("CENSO DO ANO DA QUEBRA  (maior queda de cena para cena no desvio)")
_anos = {}
for b in BLOCOS:
    _anos[b["quebra_ano"]] = _anos.get(b["quebra_ano"], 0) + 1
for _a in sorted(_anos):
    print("  %s : %2d bloco(s)%s"
          % (_a, _anos[_a], "   <- acontecimento" if _a >= "2025" else ""))
_no = [b for b in BLOCOS if b["quebra_no_acontecimento"]]
_fora = [b for b in BLOCOS if not b["quebra_no_acontecimento"]]
print()
print("  quebram em 2025-26 : %d blocos  (%d de outros beneficiários)"
      % (len(_no), sum(1 for b in _no if b["ent"] != ENT_POMAR)))
print("  quebram ANTES      : %d blocos — NÃO são exemplos deste acontecimento"
      % len(_fora))
if _fora:
    _pior = min(_fora, key=lambda z: z["degrau"])
    print("     o mais negativo de todos, %s (ENT %s, %+.4f), quebra em %s"
          % (_pior["cul"], _pior["ent"], _pior["degrau"], _pior["quebra_ano"]))
