...
FUNDO = "#fbfbfa"
# CORES. A F10 usava o mesmo vermillion para o estatuto «sem pergola» e para o
# anel do foco oriental — estatuto e identidade na mesma cor, o que a regra
# proibe. Separadas:
#   identidade  azul / laranja, a chave fixada na adenda v1.1
#   estatuto    terra, deliberadamente pouco saturada: ausencia de planta nao e
#               uma serie. Falha o piso de croma do validador de proposito, e
#               esta compensada por rotulo directo. Passa a separacao CVD
#               contra o laranja (dE 11,5 protan, 18,7 visao normal).
SEM = "#8a5a3b"
OCIDENTAL, ORIENTAL = "#2a78d6", "#eb6834"
...
# NOMES. A chave de identidade e geografica e fenologica, e a coordenada E vai
# obrigatoriamente ao lado do nome — regra permanente da adenda v1.1, adoptada
# depois de a designacao dos focos ter estado invertida e sobrevivido a quatro
# auditorias. Nenhuma valvula entra em legenda.
POS = {"foco OESTE da cadeia": (0, 118, "bottom"),
       "foco ESTE da cadeia": (0, -128, "top")}
NOMES = {"foco OESTE da cadeia": "OCIDENTAL  ·  novo, 2025",
         "foco ESTE da cadeia": "ORIENTAL  ·  crónico, desde 2017"}
for (x, y), cor, k in ((FOCO_OESTE, OCIDENTAL, "foco OESTE da cadeia"),
                       (FOCO_ESTE, ORIENTAL, "foco ESTE da cadeia")):
    ax.add_patch(Circle((x, y), 90, fill=False, ec=cor, lw=2.4, zorder=6))
    d = FOCOS[k]
    dx, dy, va = POS[k]
    txt = ("%s\nE%.0f N%.0f  ·  %s m  ·  %.0f %% acima de 1,5 m"
           % (NOMES[k], x, y, ("%.2f" % d["altura"]).replace(".", ","), d["frac"]))
...
fig.text(0.045, 0.900,
         "Altura de copado medida por LiDAR em 6 de Julho de 2025, entre as 14h35 e as 14h51 UTC. "
...
fig.text(0.045, 0.062,
         "PROVENIÊNCIA. Voo LiDAR da DGT, folhas LO-158565 e LO-159565, 16,6 e 17,8 milhões de pontos. Altura = MDS − MDT, reamostrada para a grelha de 10 m da análise.\n"
         "A DATA vem do tempo GPS dos pontos, não dos metadados — que dão uma janela inútil de catorze meses. Cálculo em disco desde 31-08-2026 (`l1_data_do_voo.py`), a pedido do adversário de 29-08:\n"
         "bit 0 de global_encoding = 1, Adjusted Standard GPS Time; 34,4 milhões de pontos nas duas folhas, todos em 06-07-2025, das 14:34:53 às 14:51:08 UTC — amplitude de 0,27 h.\n"
         "ESCALA ANCORADA em dois controlos medidos no próprio voo e não escolhidos por nós: terreno lavrado 0,09 m, referência sistemática 2,34 m.\n"
         "O QUE ESTA FIGURA NÃO FAZ: o voo cai DENTRO da janela em análise, logo não distingue «nunca teve pérgola» de «teve até Julho de 2024». Estabelece o que lá estava naquele dia, não a sua história.\n"
         "CONFIRMAÇÃO POR DOCUMENTO INDEPENDENTE: onde o LiDAR não vê pérgola, o beneficiário declarou ao IFAP erva, forragem ou nada — 65 % de kiwi, contra 99,4 % na parte com pérgola.\n"
         "NOMES: a identidade é geográfica e fenológica, com a coordenada E obrigatória ao lado. Nenhuma válvula entra em legenda — quatro reconstruções do esquema divergem 465 m na mesma válvula.",
         fontsize=7.6, color=TINTA3, linespacing=1.82, va="top")

fig.savefig(os.path.join(AQUI, "P02_dois_focos.png"), facecolor=FUNDO,
            bbox_inches="tight", pad_inches=0.32)
fig.savefig(os.path.join(AQUI, "P02_dois_focos.svg"), facecolor=FUNDO,
            bbox_inches="tight", pad_inches=0.32)
print("escrito P02 — dois focos")
