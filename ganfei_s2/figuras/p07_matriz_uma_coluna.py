# -*- coding: utf-8 -*-
"""P07 — a matriz de diagnostico tem uma coluna.

PROVENIENCIA, E A CORRECCAO QUE A PRE-VOO EXIGIU
------------------------------------------------
Deriva da F11, confrontada com o registo executavel dos factos antes de ser
composta. **Uma afirmacao nao passou.** A F11 escrevia, ao lado do ZERO das
linhas bacterianas: «a PSA — o cancro bacteriano do kiwi, a principal doenca
da cultura no mundo — nunca foi procurada.»

Isso descreve um esquecimento. Nao foi um esquecimento. Testemunho de tipo 1,
recebido a 01-09-2026 e registado em `C4_ADENDA_RAZAO_2026-09-03.md`:
**ninguem encomendou ensaio para PSA porque a sintomatologia das plantas nao
era compativel.** Pela regra dos tres tipos de facto da CLAUDE.md, testemunho
directo entra como dado e ganha ao calculo; o que ele derruba **retira-se, nao
se reconcilia**.

O zero mantem-se — continua a nao haver uma unica linha bacteriana na matriz —
mas passa a levar a sua razao ao lado. E a lacuna verdadeira passa a ser outra:
**a decisao clinica existe e nao esta escrita em lado nenhum.** Quem observou,
quando, com que criterio: nao sabido, e nao se preenche por inferencia.

O resto passou sem alteracao: as 20 linhas organismo x matriz, as 13 que
assentam numa unica amostra composta, e a coluna unica que da nome a peca.

O QUE A F11 JA DIZIA, E QUE SE MANTEM
=====================================
...
de agentes que faltam ou nunca foram pedidos ao laboratorio, ou foram
excluidos por decisao clinica que ninguem escreveu.
...
axr.text(0, 0.175, "ZERO", fontsize=22, color=GRANEL, fontweight="bold",
         va="top")
axr.text(0, 0.108,
         "linhas bacterianas ou\nvirais. A PSA — o cancro\nbacteriano do kiwi — foi\n"
         "EXCLUÍDA por sintomas\nincompatíveis. A decisão\nexiste; o registo não.",
         fontsize=8.8, color=TINTA2, va="top", linespacing=1.68)
...
fig.savefig(os.path.join(AQUI, "P07_matriz_uma_coluna.png"), facecolor=FUNDO,
            bbox_inches="tight", pad_inches=0.34)
fig.savefig(os.path.join(AQUI, "P07_matriz_uma_coluna.svg"), facecolor=FUNDO,
            bbox_inches="tight", pad_inches=0.34)
print("escrito P07 — contagem por coluna:", cont)
